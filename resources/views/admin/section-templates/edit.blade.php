...
@section('content')
    <ul class="breadcrumb breadcrumb-top">
        ...
        <li><a href="{{ route('admin.section-templates.show', $template->id) }}">{{ $template->name }}</a></li>
        <li><a href="">Edit</a></li>
    </ul>
    ...
    {{  Form::open([
        'method' => 'PUT',
        'id' => 'template-form',
        'route' => ['admin.section-templates.update', $template->id],
        ...
        <div class="col-md-12">
            <div class="block">
                <div class="block-title">
                    <h2><i class="fa fa-pencil"></i> <strong>Edit Template "{{ $template->name }}"</strong></h2>
                </div>
                ...
                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="name" name="name" class="form-control" 
                               value="{{ old('name', $template->name) }}" required placeholder="Enter template name...">
                        @if($errors->has('name'))
                ...
                <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="slug" name="slug" class="form-control" 
                               value="{{ old('slug', $template->slug) }}" placeholder="Leave blank to auto-generate">
                ...
                <div class="form-group{{ $errors->has('category') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="category" name="category" class="form-control" 
                               value="{{ old('category', $template->category) }}" placeholder="e.g., Content, Media, Layout">
                ...
                <div class="form-group{{ $errors->has('icon') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-9">
                        <input type="text" id="icon" name="icon" class="form-control" 
                               value="{{ old('icon', $template->icon) }}" placeholder="e.g., fa-puzzle-piece">
        ...
        <div class="col-md-12">
            <div class="block">
                ...
                <div id="fields-container">
                    <div id="fields-list" style="{{ $template->fields->count() > 0 ? '' : 'display: none;' }}">
                        @foreach($template->fields->sortBy('sort_order') as $index => $field)
                            <div class="field-item well" style="margin-bottom: 15px; position: relative; border: 1px solid #ddd;" data-field-id="{{ $index + 1 }}">
                                <div class="field-header" style="margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #eee;">
                                    <div class="pull-left">
                                        <span class="field-type-badge label label-primary">{{ $field->fieldType->name }}</span>
                                        <strong class="field-name" style="margin-left: 10px;">{{ $field->name }}</strong>
                                    </div>
                                ...
                                <div class="field-config form-horizontal">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" name="fields[{{ $index + 1 }}][name]" class="form-control field-name-input" 
                                                   value="{{ old('fields.' . ($index + 1) . '.name', $field->name) }}" required />
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" name="fields[{{ $index + 1 }}][alias]" class="form-control field-alias-input" 
                                                   value="{{ old('fields.' . ($index + 1) . '.alias', $field->alias) }}" required />
                                            <span class="help-block">Used in templates</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" name="fields[{{ $index + 1 }}][label]" class="form-control" 
                                                   value="{{ old('fields.' . ($index + 1) . '.label', $field->label) }}" />
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input type="text" name="fields[{{ $index + 1 }}][placeholder]" class="form-control" 
                                                   value="{{ old('fields.' . ($index + 1) . '.placeholder', $field->placeholder) }}" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <textarea name="fields[{{ $index + 1 }}][help_text]" class="form-control" rows="2">{{ old('fields.' . ($index + 1) . '.help_text', $field->help_text) }}</textarea>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            <input type="hidden" name="fields[{{ $index + 1 }}][is_required]" value="0">
                                            <label class="switch switch-primary">
                                                <input type="checkbox" name="fields[{{ $index + 1 }}][is_required]" value="1" {{ $field->is_required ? 'checked' : '' }}>
                                                <span></span>
                                            </label>
                                        </div>
                                    </div>
                                    
                                    <input type="hidden" name="fields[{{ $index + 1 }}][field_type_id]" 
                                           class="field-type-id" value="{{ $field->field_type_id }}" />
                                    <input type="hidden" name="fields[{{ $index + 1 }}][sort_order]" 
                                           class="sort-order" value="{{ $field->sort_order }}" />
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div id="no-fields-message" class="text-center" style="{{ $template->fields->count() > 0 ? 'display: none;' : '' }} padding: 40px 0;">
        ...
        <div class="col-md-12">
            <div class="form-group form-actions">
                <div class="col-md-9 col-md-offset-3">
                    <button type="submit" class="btn btn-sm btn-primary"><i class="fa fa-save"></i> Update Template</button>
                    <a href="{{ route('admin.section-templates.show', $template->id) }}" class="btn btn-sm btn-default">View Template</a>
